    ...
    <div className="flex flex-col lg:flex-row h-screen w-screen bg-[#0f172a] text-white font-sans">
      ...
      <header className="lg:hidden bg-[#1e293b] border-b border-slate-700 z-50">
        ...
        <div className={`${isMobileSearchOpen ? 'block' : 'hidden'} px-4 pb-3`}>
          <input
            type="text"
            placeholder="Ara..."
            className="w-full bg-[#0f172a] border border-slate-700 rounded-lg py-2.5 px-4 text-sm focus:outline-none focus:border-[#FF9900]"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
          {searchTerm && filteredSections.length === 0 && (
            <p className="text-sm text-slate-400 mt-2">Sonuç bulunamadı.</p>
          )}
        </div>
      </header>
      ...
      <aside className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:static top-14 lg:top-0 left-0 w-80 h-[calc(100vh-56px)] lg:h-screen bg-[#1e293b] border-r border-slate-700 transition-transform duration-300 z-40 flex flex-col`}>
        <div className="p-6 flex-1 flex flex-col overflow-hidden">
          <div className="hidden lg:flex flex-col items-center mb-8 cursor-pointer"
            onClick={() => handlePageChange(null)}>
          ...
          <div className="relative mb-6 hidden lg:block">
            ...
            <input
              type="text"
              placeholder="Ara..."
              className="w-full bg-[#0f172a] border border-slate-700 rounded-lg py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:border-[#FF9900]"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          ...
          <div className="mt-6 pt-4 border-t border-slate-700 text-center text-[10px] text-slate-500">
            © 2026
            <a href="https://github.com/BozgunBer-2506" target="_blank" rel="noopener noreferrer" className="text-[#FF9900] hover:underline ml-1">The_Bozgun</a>
          </div>
        </div>
      </aside>
          ...
          <article className="max-w-4xl mx-auto space-y-6">
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                summary: ({ children }) => <summary className="flex items-center justify-between cursor-pointer px-4 py-3 font-bold text-[#FF9900] list-none">
                  <div className="flex items-center gap-2 text-xs font-black uppercase"><GraduationCap size={16} /><span>CEVABI GÖR</span></div>
