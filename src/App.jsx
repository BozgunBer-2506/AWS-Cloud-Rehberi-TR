...
import { ChevronRight, ChevronLeft, ChevronUp, Search, GraduationCap, Maximize2, Minimize2, Github } from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-screen bg-[#0f172a] text-white font-sans overflow-hidden">
      ...
      <aside className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${isCollapsed ? 'lg:hidden' : 'lg:flex'} fixed lg:static top-14 lg:top-0 left-0 w-80 h-[calc(100vh-56px)] lg:h-screen bg-[#1e293b] border-r border-slate-700 transition-transform z-40 flex flex-col overflow-hidden`}>
        <div className="p-6 flex-1 flex flex-col overflow-hidden">
          ...
          <div className="mt-6 pt-4 border-t border-slate-700 text-center text-[10px] text-slate-500">
            <a href="https://github.com/BozgunBer-2506" target="_blank" rel="noopener noreferrer" className="text-[#FF9900] font-bold tracking-widest">The_Bozgun</a>
            <br />
            ...
            <br />
            <span className="mt-1 inline-block">© 2026</span>
          </div>
        </div>
      </aside>

      <main className="flex-1 overflow-y-auto px-6 py-10 relative">
        <button onClick={() => setIsCollapsed(!isCollapsed)} className="hidden lg:flex fixed top-6 right-8 z-[70] items-center gap-2 bg-slate-800/80 hover:bg-[#FF9900] text-slate-300 hover:text-white px-3 py-1.5 rounded-full border border-slate-700 transition-all shadow-xl backdrop-blur-sm">
          {isCollapsed ? <Maximize2 size={16} /> : <Minimize2 size={16} />}
          <span className="text-[10px] font-bold uppercase tracking-widest">{isCollapsed ? 'Genişlet' : 'Odak Modu'}</span>
        </button>
        ...
        {selectedSection ? (
          <article className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="prose prose-invert max-w-none">
              <ReactMarkdown
                ...
                components={{
                  ...
                  h2: ({ children }) => <h2 className="text-2xl lg:text-3xl font-semibold mt-6 mb-3 border-b border-slate-800 pb-2">{children}</h2>,
                  h3: ({ children }) => <h3 className="text-xl lg:text-2xl font-semibold mt-4 mb-2 text-[#FF9900]">{children}</h3>,
                  p: ({ children }) => <p className="text-base lg:text-lg text-slate-300 mb-4 leading-relaxed">{children}</p>,
                  ...
                  summary: ({ children }) => (
                    <summary className="flex items-center justify-between cursor-pointer px-4 py-3 font-bold text-[#FF9900] list-none hover:bg-slate-800/50">
                      <div className="flex items-center gap-2 text-xs font-black uppercase"><GraduationCap size={16} />{children}</div>
                      <ChevronRight size={16} />
                    </summary>
                  )
                }}
            ...
            <div className="flex justify-between pt-10 border-t border-slate-800">
              <button onClick={handlePrevPage} disabled={allSections.findIndex(s => s.id === selectedSection.id) === 0} className="flex items-center gap-2 text-sm text-slate-500 hover:text-[#FF9900] disabled:opacity-0 transition">
                <ChevronLeft size={16} /> Önceki Bölüm
              </button>
              <button onClick={handleNextPage} disabled={allSections.findIndex(s => s.id === selectedSection.id) === allSections.length - 1} className="flex items-center gap-2 text-sm text-[#FF9900] hover:text-orange-400 disabled:opacity-0 transition">
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            ...
            <h3 className="text-2xl text-[#FF9900] font-bold mb-8 uppercase tracking-widest">Eğitim Rehberi</h3>
            <p className="text-lg text-slate-400">Başlamak için yan menüden bir konu seçin.</p>
          </div>
        )}

        {selectedSection && (
          <button onClick={handleScrollToTop} className="fixed bottom-6 right-6 bg-[#FF9900] text-[#1e293b] p-3 rounded-lg hover:bg-orange-400 transition shadow-2xl z-50">
            <ChevronUp size={20} />
          </button>
        )}
      </main>
